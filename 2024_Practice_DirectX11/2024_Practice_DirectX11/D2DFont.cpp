...
void D2DFont::DrawSolidText(D2DUIConfig::FontSize size, D2D1::ColorF color, const char* text, DirectX::XMFLOAT2 pos,
...
void D2DFont::DrawRoundRect(D2D1_ROUNDED_RECT roundRect)
{
	ID2D1RenderTarget* renderTarget = gD3D->Get2DRenderTarget();
	renderTarget->BeginDraw();
	renderTarget->FillRoundedRectangle(roundRect,mSolidBrush.Get());
	renderTarget->EndDraw();
}

void D2DFont::DrawRoundRectOutline(D2D1_ROUNDED_RECT roundRect)
{
	ID2D1RenderTarget* renderTarget = gD3D->Get2DRenderTarget();
	renderTarget->BeginDraw();
	renderTarget->DrawRoundedRectangle(roundRect, mSolidBrush.Get());
	renderTarget->EndDraw();
}
